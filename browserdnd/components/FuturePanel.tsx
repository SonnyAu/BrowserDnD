...
export default function FuturePanel({ inventory, showInventory, selectedInventoryId, onSelectItem }: FuturePanelProps) {
  if (!showInventory) {
    return (
  ...
  return (
    <div className="flex h-full flex-col rounded-lg border border-[#3d2f23] bg-[linear-gradient(165deg,#17120f,#120e0c)] p-3 shadow-[0_0_20px_rgba(0,0,0,0.45)]">
      ...
      <p className="mt-1 text-[11px] uppercase tracking-wide text-[#e8d9b5]/60">Select item, then click an Armory slot to equip.</p>
      <div className="mt-3 space-y-2 overflow-y-auto">
        {inventory.length === 0 && <p className="text-xs text-[#e8d9b5]/55">No items.</p>}
        {inventory.map((item) => (
          <button key={item.id} onClick={() => onSelectItem(item.id)} className={`w-full rounded border px-3 py-2 text-left text-xs uppercase tracking-wide ${selectedInventoryId === item.id ? "border-[#d9b66f] bg-[#2b1f17]" : "border-[#3d2f23] bg-[#140f0d]"}`}>
